...
try {
    ...
    else { Write-Fail "Violacoes encontradas (veja acima)" }
} catch {
...
try {
    python manage.py test --verbosity=1 2>&1
    if ($LASTEXITCODE -eq 0) { Write-Pass "Todos os testes passaram" }
    else { Write-Fail "Testes falharam (veja acima)" }
} catch {
...
try {
    ...
    else { Write-Fail "Deploy check encontrou problemas (veja acima)" }
} catch {
