...
4. ~~Custo de equipamentos travado ao catálogo~~ ✅ feito em 2026-08-16 —
   `valor_equipamentos_manual` deixa o vendedor digitar o valor cotado pelo
   fornecedor sem depender de preço vigente cadastrado pra cada item.
5. **Popular o catálogo** — 👉 **é aqui que estamos.** Já não trava mais o
   *valor* da proposta (item 4), mas a **tabela de equipamentos do PDF**
   ainda mostra só o que está cadastrado — cadastrar estrutura de fixação,
   cabos, conectores, DPS e disjuntores em `/solar/estruturas/` e
   `/solar/materiais/`, com preço em `/solar/precos/`, deixa o PDF completo
   de verdade (e o aviso do item 2 some sozinho quando isso acontecer).
...
| **Catálogo ainda incompleto** | 4 módulos + 31 inversores (todos com preço vigente real da Belenus, 2026-08-16), 1 estrutura, 2 materiais elétricos. Sem estrutura de fixação, cabos e conectores suficientes cadastrados, a tabela de equipamentos do PDF fica incompleta — mas o **valor** da proposta já não depende mais disso, ver linha abaixo. Caminho que funcionou pra alimentar preço real: sessão assistida via Claude in Chrome, usuário logado no portal do fornecedor — ver DIARIO 2026-08-16k/l |
| **Garantia de 30 inversores está com valor conservador "a confirmar"** | 🟡 Dado a validar | Belenus não publica garantia de forma consistente entre marcas (só a Deye tinha o texto legal na página). Fontes públicas divergem por marca — usei o número mais conservador documentado (Auxsol/Huawei/Deye = 10 anos, Growatt/Solis/Sungrow = 5 anos). Vale confirmar com nota fiscal ou contato direto com cada fabricante antes de prometer esse número numa proposta importante. `tensao_max_entrada` de todos os 30 é um placeholder (500V) — não afeta cálculo nem aparece no PDF, mas não é dado real |
| ~~**Composição de preço incompleta**~~ | ✅ Contornado (2026-08-16). `valor_equipamentos_manual` deixa o vendedor digitar o valor total cotado pelo fornecedor direto, sem depender de cada item ter preço vigente no catálogo. Não substitui popular o catálogo (a tabela de itens no PDF ainda precisa dos itens certos), mas destrava a proposta enquanto isso não acontece |
