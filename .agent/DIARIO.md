...
- `[2026-08-18b]` **Fluxo Belenus documentado + diagnóstico comparativo.** Usuário mandou prints do fluxo completo "Gerador Personalizado" da Belenus (6 passos: Módulo → Inversor → Estrutura → Componentes Elétricos CC/CA → Confirmação) e deu o veredito: *"O fluxo é muito semelhante mas a experiência da Intelbras é mais agradável."* Documentei o fluxo inteiro em `.agent/referencia-ux-propostas-solares.md` §2 e escrevi um diagnóstico (§3) apontando 3 diferenças estruturais entre os dois, não só estética: (1) Intelbras pede o cliente **primeiro**, Belenus só no passo final — sensação de "montar proposta de alguém" vs "configurar produto"; (2) Intelbras foi construída como ferramenta de gestão (funil de vendas, dashboard, 1.292 projetos), Belenus reaproveita a engine de e-commerce da própria loja (carrinho, cupom, "Comprar Agora"); (3) a caixa "Premissas de Cálculo" da Intelbras fica fixa em toda tela — transparência de metodologia sempre visível. Achado que se repete nos dois fornecedores (logo não é peculiaridade de um só): o motor de sugestão automática de materiais de fixação a partir do arranjo físico de módulos — reforça que essa é a peça de maior prioridade se decidirmos atacar o catálogo vazio de estrutura/material. Recomendação registrada: manter `PropostaSolar.cliente` como primeiro campo do form em qualquer redesenho (já é assim hoje) — bate com o que os dois fornecedores fazem de melhor, não é escolha a questionar. Nenhuma mudança de código — só síntese, à espera de decisão do usuário sobre o que priorizar.
- `[2026-08-18]` **Pesquisa de UX: fluxo de propostas da Intelbras, referência pro nosso formulário.** Usuário quer imitar a lógica e o layout de preenchimento de proposta da Intelbras (e em seguida da Belenus) — não por estética, mas porque a equipe já está habituada a esse fluxo e ele considera a lógica bem construída. Percorri o wizard completo via sessão assistida (Claude in Chrome): 5 abas (Configurações → Módulos → Inversores → Solução → Valor Total), cada uma com checkmark ao validar. Achados que validam decisões já tomadas: sigla `ETO` pra concessionária (mesma da ANEEL), sugestão de simultaneidade "25% residencial e 50% comercial" (bate com a skill), tipo de proposta "Apenas gerador sem instalação" (confirma a separação equipamento/instalação do modelo de repasse). Achado de maior potencial: a tela "Disposição dos módulos e fileiras" (retrato/paisagem, fileiras, tipo de telha) alimenta automaticamente uma lista de materiais sugeridos — um motor de "receita" de BOM que resolveria o problema do catálogo vazio de estrutura/material sem depender só de cadastro manual. **Confirmado**: a tarifa auto-preenchida pela Intelbras (R$ 0,98/kWh pra ETO) está desatualizada frente à fatura real (R$ 1,38). Documentado por completo em `.agent/referencia-ux-propostas-solares.md` (seção Belenus ainda vazia — usuário vai mostrar em seguida). Nenhuma mudança de código nesta entrada — é registro de pesquisa, decisão de implementação fica pra depois.
- `[2026-08-16l]` **30 inversores cadastrados via sessão assistida — e um achado sobre qualidade de dado de fornecedor.** Usuário pediu pra cadastrar os inversores da categoria monofásico 220V da Belenus, avisando que eram "muitos" — eram 30 (Auxsol, Deye, Growatt, Huawei, Solis, Sungrow, 3 a 10kW). Dois achados mudaram o plano original: (1) a página de detalhe da Belenus **não publica** tensão máxima de entrada nem o modelo real do fabricante — só o código Belenus e specs físicas; confirmado por grep que `tensao_max_entrada` nunca é lido em cálculo nem aparece no PDF do cliente, então usei o código Belenus como `modelo` e um placeholder documentado (500V) pra tensão, sem gastar tempo abrindo 30 datasheets em PDF. (2) Garantia **é inconsistente entre marcas na própria Belenus** — a Deye tem um bloco de texto legal completo (10 anos, 5+5) direto na página, a Auxsol não tem nada. Como garantia aparece no PDF do cliente (diferente da tensão), tentei resolver via busca pública e as fontes **divergiam de verdade** entre si (Growatt: "2 anos da instalação" vs "5 anos" conforme a fonte; Solis: 5 ou 7 anos dependendo da data de fabricação; Sungrow: "5 a 10 anos conforme a série"). Decisão do usuário: usar o número mais conservador documentado por marca, sinalizado como "a confirmar" no comentário do seed. Preço de todos os 30 é real, direto da Belenus logada (não estimado). Catálogo de inversores: 1 → 31, todos com preço vigente.
- `[2026-08-16k]` **Primeira sessão assistida via Claude in Chrome — preço real dos 3 módulos.** Usuário perguntou se dava pra automatizar leitura dos portais logados dos fornecedores (Intelbras, Belenus) pra manter o catálogo em dia; a resposta foi não guardar credencial pra automação recorrente não-assistida (risco de segurança + provável violação de ToS de portal B2B), mas sim usar o Claude in Chrome pra ler páginas com o usuário logado e presente, sessão por sessão. Testado agora pela primeira vez: usuário logou na Belenus na aba controlada pela ferramenta, e a listagem de "Painel Fotovoltaico" trouxe os 3 preços reais direto — RONMA R$ 479,70, TCL R$ 540,59, Astronergy R$ 693,55 (esse último batendo exatamente com o valor já visto numa captura de tela anterior). `PrecoEquipamentoSolar` criado pros 3 com `preco_custo == preco_venda` (decisão consistente com o modelo de negócio real: Optimus não tem margem sobre equipamento). Catálogo de módulos: todos os 4 ativos agora têm preço vigente.
- `[2026-08-16j]` Cadastrados 3 módulos novos no catálogo (RONMA RM-585W-182M/144TB, TCL Solar HSM-ND66-GR615, Astronergy CHSM66N(DG)/FBH-715) a partir de uma captura de tela de portal de fornecedor — usuário esclareceu que a Intelbras não vende linha solar naquela tabela (só CFTV etc). Specs elétricas e dimensões vieram de datasheet oficial do fabricante (potência, eficiência, Voc, Isc, área — tudo confirmado, nenhum inventado); garantia (15/30 anos) e peso do RONMA/TCL vieram de convenção de mercado ou revenda, não da ficha oficial — sinalizado ao usuário para conferir contra a nota fiscal real. Catálogo de módulos: 1 → 4.
- `[2026-08-16i]` **"Custo Base de Equipamentos" virou editável.** Pedido direto do usuário, mas a motivação de fundo é a descoberta do dia anterior: como a Optimus não precifica equipamento (venda direta, sem margem — ver entrada abaixo), e o catálogo interno tem só 5 itens, exigir que o valor da proposta viesse 100% da soma de preços cadastrados travava qualquer proposta com estrutura/cabo sem preço vigente. Novo campo `PropostaSolar.valor_equipamentos_manual` (nullable): quando preenchido, sobrepõe a soma calculada dos itens (`valor_equipamentos_calculado`, que continua existindo à parte). O antigo `<div>` somente-leitura no formulário virou `<input>` editável; o JS que recalculava ao vivo parou de escrever direto no campo (apagaria o que o vendedor digitou a cada item alterado) e passou a oferecer um link "usar este valor" ao lado. Endpoint `calcular-total/` mudou de texto puro pra JSON (`{"formatado", "raw"}`) pra evitar parsing frágil de string formatada no cliente. 15 testes novos, 229 no total.
